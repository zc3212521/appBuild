...
const commonFun = {
  ...
  Validation: {
    ...
    isPassword: function(str) {
      var isPassReg = /^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6,10}$/;
      return isPassReg.test(str);
    },
    isIDCard: function(str) {

      var idCardReg = /^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|X)$/gi;
    ...
    isURL: function(str) {
      var regexp = /(http|ftp|https):\/\/[\w\-_]+(\.[\w\-_]+)+([\w\-\.,@?^=%&:/~\+#]*[\w\-\@?^=%&/~\+#])?/i;
      return regexp.test(str);
    },
    isLiveURL: function(str) {
      var regexp = /^[http|https].*\/{2,}.*$/g;
      return regexp.test(str);
    },
    isQQ: function() {
    ...
    isImgURL: function() {
      var myReg = /^\.(jpeg|jpg|gif|png)$/;
      if(myReg.test(this)) return true;
      return false;
    },
    getPwdStrength: function(pwd) {
      var array = [];
      array[0] = pwd.match(/^.{8,49}$/);
      array[1] = pwd.match(/[A-Z]/);
      array[2] = pwd.match(/[a-z]/);
      array[3] = pwd.match(/\d/);
      array[4] = pwd.match(/[!_.-]/);
  ...
  Datetime: {
    ...
    yyyymmddhhmm: function(D) {
      ...
      return "".concat(yyyy).concat("-").concat(mm).concat("-").concat(dd).concat(" ").concat(hh).concat(":").concat(
        min);
    },
  ...
  handleLabel: function(rule, value, callback) {   // 标签处理，提取为同一个函数
    ...
    returnTableHTML: function(data,title,colNames,dataKey){ //将数据转换成表格html
        if(data instanceof Array && data.length>0){
            let colSum = dataKey.length; // 列宽
            console.log("colSum: ",colSum)
            let titleHTML = "<tr><th style='text-align: center;font-size: 20px' colspan='"+ (colSum+1) +"'>" + title + "</th></tr>"
            if (colNames instanceof Array && colSum === colNames.length){
                let threadHTML = "<tr><th  style='background-color: #c0c0c0;'>编号</th>\n",
                    trHTML = "";
                for(let i=0; i<colSum; i++){
                threadHTML += "<th  style='background-color: #c0c0c0;'>"+ colNames[i] +"</th>\n"
                }
                threadHTML += "</tr>\n";

                for(var i=0; i<data.length; i++){
                  trHTML += "<tr><td>"+ (i+1) + "</td>\n";
                  for(var n=0; n<colSum; n++){
                    trHTML += "<td>" + data[i][dataKey[n]] + "</td>\n";
                  }
                  trHTML += "</tr>\n"
                }
                let html = "<html><head><meta charset='utf-8' /></head><body><table border='1'>" + titleHTML + threadHTML + trHTML + "</table></body></html>";
                return html;
            }else{
              console.error("请检查表格列数与数据展示数是否一致")
                return false
            }
        }else{
          message.error("表格数据为空或有误，请刷新重试。")
            return false
        }
    },
    toExcel:function(data,title,colNames,dataKey){
      let html = this.returnTableHTML(data,title,colNames,dataKey);
      if(html){
        // 实例化一个Blob对象，其构造函数的第一个参数是包含文件内容的数组，第二个参数是包含文件类型属性的对象
        let blob = new Blob([html], { type: "application/vnd.ms-excel" });
        let a = document.getElementById("toExcel");
